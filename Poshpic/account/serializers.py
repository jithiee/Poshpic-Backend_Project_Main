...
class RegisterSerializer(serializers.ModelSerializer):
        ...
        class Meta:
            model = User
            fields = [ 'username','email','is_photographer','is_user','password','confirm_password','is_verified','otp']
        ...
        def create(self, validated_data):   
            ...
            validated_data.pop('confirm_password', None) # pop is dict method None is default , confirm_password don't store in database
...
class PhotogrpherProfileSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = ['specialty','experience','city','country','status','address','phone','profile_image']     
        
class Userserializer(serializers.ModelSerializer):
    userData = UserProfileSerializer(source='userprofile', required=False)
    PhotographerData = PhotogrpherProfileSerializer(source='photographerprofile', required=False)
    ...
    def update(self, instance, validated_data):
        ...
        if userprofile_data:
            userprofile_instance, created = Userprofile.objects.get_or_create(user=instance)
        ...
        if photographer_data:
            photographer_instance, created = PhotographerProfile.objects.get_or_create(user=instance)
